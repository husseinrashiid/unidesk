...
A release keystore already exists on this machine at `.local/keystore/unidesk-release.keystore`
(alias `unidesk-release`, PKCS12, RSA 4096, valid to 2056); its passwords are in the gitignored
`.local/keystore/README-KEEP-THIS.txt`. Back that file and the keystore up somewhere durable and
outside this machine (password manager, encrypted offline backup) — losing them means future
updates can never be installed over this app identity again. `src-tauri/gen/android/app/build/outputs/apk/arm64/release/app-arm64-release.apk`
is the current signed build; it verifies with `apksigner verify --verbose --print-certs`.
...
Native startup uses a bounded, read-only readiness probe before initial SQLite queries. No mutation is replayed by that probe. See [the verification report](docs/platform-expansion-report.md) for measured results.
