...
`server/sync-service.ts` and `scripts/sync-account.mjs` remain protocol fixtures for testing and legacy compatibility. The app's account UI targets Supabase. See [Android builds](android.md) for builds and device behavior.
